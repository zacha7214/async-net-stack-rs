 ...
 *
 * Sourced from the Linux kernel, samples/bpf/xdpsock_user.c @ v5.19 (the
 * last kernel release to carry this sample; it was removed in v6.0 in favor
 * of xdp-tools/xsk-router). Local modifications for building against
 * libbpf >= 1.0 and libxdp (xdp-tools v1.6.3) rather than the in-kernel
 * libbpf/xsk helpers:
 *
 *   1. <bpf/xsk.h>          -> <xdp/xsk.h>
 *   2. bpf_xdp_query_id()   -> xdp_link_query_id()  (netlink, see xdp_link.c)
 *   3. bpf_xdp_detach()     -> xdp_link_detach()
 *   4. bpf_prog_load_xattr()+bpf_xdp_attach() -> bpf_object__open/load +
 *      xdp_link_attach()
 *   5. libbpf_set_strict_mode() -> compiled out on libbpf >= 1.0
 *   6. added fallback #define for XDP_UMEM_UNALIGNED_CHUNK_FLAG
 *      (older kernel UAPI headers)
 */
...
static void remove_xdp_program(void)
{
	...
	if (xdp_link_query_id(opt_ifindex, opt_xdp_flags, &curr_prog_id)) {
		printf("xdp_link_query_id failed\n");
		exit(EXIT_FAILURE);
	}

	if (prog_id == curr_prog_id)
		xdp_link_detach(opt_ifindex, opt_xdp_flags);
						    ...
						    bool rx, bool tx)
{
	...
	ret = xdp_link_query_id(opt_ifindex, opt_xdp_flags, &prog_id);
	if (ret)
		exit_with_error(-ret);
...
/* Patched: bpf_prog_load_xattr() and bpf_xdp_attach() were removed from
 * libbpf 1.0; use the object-level API plus a plain netlink attach helper
 * (see xdp_link.c). Only used for -M (shared UMEM / multiple sockets).
 */
static void load_xdp_program(char **argv, struct bpf_object **obj)
{
	...
	*obj = bpf_object__open(xdp_filename);
	...
	ret = xdp_link_attach(opt_ifindex, prog_fd, opt_xdp_flags);
	if (ret < 0) {
		fprintf(stderr, "ERROR: link set xdp fd failed: %s\n",
			strerror(-ret));
		exit(EXIT_FAILURE);
	}
}
...
int main(int argc, char **argv)
{
	...
	if (opt_reduced_cap) {
		...
		/* libbpf < 1.0: request the strict mode that 1.0 enables by default */
